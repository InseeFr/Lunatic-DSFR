 ...
 */
export async function expectLunaticData(page: Page, propertyPath: string, expected: unknown) {
    const consoleOut = page.waitForEvent("console");
    await page.getByRole("button", { name: "Get Data" }).click();
    const output = await consoleOut;
    expect(await output.args()[0].jsonValue()).toHaveProperty(propertyPath, expected);
}

/**
 * Check the output of getData() from lunatic
 */
export async function expectCollectedData(page: Page, name: string, expected: unknown) {
