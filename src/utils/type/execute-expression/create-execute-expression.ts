...
import { COLLECTED, VTL, VTL_MD, X_AXIS, Y_AXIS } from "../../../utils/constants";
...
function createBindings(variables: LunaticState["variables"]): Bindings {
    return Object.entries(variables).reduce(function (bindings, [name, { value }]) {
 ...
 */
function createExecuteExpression(variables: LunaticState["variables"], features: string[]) {
     ...
     */
    function setLoopBindings(variables: LunaticState["variables"], iteration: number) {
    ...
    function execute<T = unknown>(
        ...
        const { value: expression, type } = validateExpression(getSafetyExpression(expObject));
